/**
 * Tiny UTF-8 / QR Code Generator for JavaScript
 * Adapted from standard qr code generator implementations
 */
var QRCode = (function() {
  // QRCode constructor
  function QRCode(element, options) {
    ...
    this._htOption = {
      ...
      colorDark: "#000000",
      colorLight: "#ffffff",
      correctLevel: 1 // QRCoder.CorrectLevel.H
    };
    ...
    this._oQRCode = null;
    this._oQRCode = new QRCodeModel(LimitLengthTable(this._htOption.typeNumber, this._htOption.correctLevel), this._htOption.correctLevel);
  ...
  QRCode.prototype.makeImage = function() {
    ...
    elCanvas.width = this._htOption.width;
    elCanvas.height = this._htOption.height;
  ...
  QRCodeModel.prototype = {
    ...
    setupPositionProbePattern: function(r, c) {
      for (var row = -1; row <= 7; row++) {
        if (r + row < 0 || this.moduleCount <= r + row) continue;
        for (var col = -1; col <= 7; col++) {
          if (c + col < 0 || this.moduleCount <= c + col) continue;
        ...
        while (true) {
          for (var c = 0; c < 2; c++) {
            ...
            if (this.modules[row][targetCol] == null) {
              ...
              var mask = QRUtil.getMask(maskPattern, row, targetCol);
              if (mask) {
                dark = !dark;
              }
  ...
  QRCodeModel.createBytes = function(buffer, rsBlocks) {
    ...
    var maxDcCount = 0;
    var maxEcCount = 0;
    var dcValues = new Array(rsBlocks.length);
    var ecValues = new Array(rsBlocks.length);
    for (var r = 0; r < rsBlocks.length; r++) {
      ...
      for (var i = 0; i < dcValues[r].length; i++) {
        dcValues[r][i] = 0xff & buffer.buffer[i + offset];
      }
      offset += dcCount;
      var rsPoly = QRUtil.getErrorCorrectPolynomial(ecCount);
      var rawPoly = new QRPolynomial(dcValues[r], rsPoly.getLength() - 1);
      var modPoly = rawPoly.mod(rsPoly);
      ecValues[r] = new Array(rsPoly.getLength() - 1);
      for (var i = 0; i < ecValues[r].length; i++) {
        var modIndex = i + modPoly.getLength() - ecValues[r].length;
        ecValues[r][i] = (modIndex >= 0) ? modPoly.get(modIndex) : 0;
      }
    }
  ...
  QRRSBlock.getRsBlockTable = function(typeNumber, errorCorrectLevel) {
    // Basic table subset for simplicity and small size (covers type 1-4)
    var idx = (typeNumber - 1) * 4;
    switch (errorCorrectLevel) {
      case QRErrorCorrectLevel.L: return [1, QRRSBlock.RS_BLOCK_TABLE[idx][1], QRRSBlock.RS_BLOCK_TABLE[idx][2]];
      case QRErrorCorrectLevel.M: return [1, QRRSBlock.RS_BLOCK_TABLE[idx+1][1], QRRSBlock.RS_BLOCK_TABLE[idx+1][2]];
      case QRErrorCorrectLevel.Q: return [1, QRRSBlock.RS_BLOCK_TABLE[idx+2][1], QRRSBlock.RS_BLOCK_TABLE[idx+2][2]];
      case QRErrorCorrectLevel.H: return [1, QRRSBlock.RS_BLOCK_TABLE[idx+3][1], QRRSBlock.RS_BLOCK_TABLE[idx+3][2]];
      default: return null;
    }
  };
  ...
  function LimitLengthTable(typeNumber, errorCorrectLevel) {
    // Enforce type 4 for maximum URL compatibility and size
    return 4;
  }
